...
IGA iga;
IWB iwb;
bool iwa_found = false, iia_found = false, iga_found = false, iwb_found = false;
...
void loop() {
  ...
  double b, t2;
  if (iwb_found && iwb.getData(b, t2)) {
    sprintf(buffer, "%s,%.2f,%.2f", buffer, b, t2);
  } else {
    Serial.println("Failed to get IWB data.");
  }
  uint16_t co2, tvoc;
  if (iga_found && iga.getData(co2, tvoc)) {
    sprintf(buffer, "%s,%d,%d", buffer, co2, tvoc);
  } else {
