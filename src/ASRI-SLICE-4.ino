...
#include <IGA.h> //weather
IGA iga;

const char* columnHeadings = "Time (ms), Accel_X (g), Accel_Y (g), Accel_Z (g), Pressure (hPa), Temperature (C), Humidity(RH),  TemperatureWeather (C)" ;
...
void loop() {
 ...
 uint16_t RH, temperature;
  delay (10) ;
  if (iga.getData(RH, temperature)) {
